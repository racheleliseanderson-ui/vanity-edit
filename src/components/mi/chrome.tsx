      ...
      <div className="mx-auto max-w-[1400px] px-5 py-3 md:flex md:items-center md:justify-between md:gap-6 md:px-10 md:py-4">
        ...
        <div className="mt-2 flex items-center gap-2 md:mt-0 md:gap-4">
          <nav
            ...
            className="-mx-1 flex flex-1 items-center gap-1 overflow-x-auto px-1 [scrollbar-width:none] md:flex-none md:gap-2 [&::-webkit-scrollbar]:hidden"
          >
            {NAV.map((n) => (
              <Link
                key={n.to}
                ...
                className="tap flex shrink-0 items-center whitespace-nowrap rounded-full px-3 text-[0.7rem] tracking-[0.14em] uppercase text-muted-foreground transition-colors hover:text-foreground active:opacity-70 data-[status=active]:text-champagne md:text-[0.78rem]"
              >
                {t(n.key as Key)}
              </Link>
            ))}
